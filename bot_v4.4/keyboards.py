...
def _settings_menu(prefix: str, back_cb: str) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        _noop("── Сигналы ──────────────────────────────────────────────────"),
        _btn("⚡ SMC условия входа",             "menu_" + p + "smc"),
...
# ── SMC УСЛОВИЯ ────────────────────────────────────────────────────────────
#
#  Формат: описание на нон-кликабельной строке, кнопка — короткая и чёткая.
#  Так текст всегда влезает и читается.

def _smc_kb(cfg: TradeCfg, prefix: str, back_cb: str) -> InlineKeyboardMarkup:
    p = prefix
    rows = [
        _noop("⚡ SMC CONDITIONS — включай только нужные"),
        _noop("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"),

        # BOS
        _noop("📌 BOS — первый пробой уровня структуры. Подтверждает"),
        _noop("   что рынок сломал предыдущий хай/лоу и началось движение."),
        _btn(_check(cfg.smc_use_bos) + "  BOS — Break of Structure", p + "smc_toggle_bos"),

        _noop("─────────────────────────────────────────"),

        # Order Block
        _noop("📌 OB — зона где крупный игрок набирал позицию перед"),
        _noop("   сильным движением. Цена часто возвращается в OB."),
        _btn(_check(cfg.smc_use_ob) + "  Order Block (OB)", p + "smc_toggle_ob"),

        _noop("─────────────────────────────────────────"),

        # FVG
        _noop("📌 FVG — дисбаланс (имбаланс) между свечами: рынок"),
        _noop("   прошёл зону так быстро что оставил «пустоту»."),
        _btn(_check(cfg.smc_use_fvg) + "  FVG — Fair Value Gap", p + "smc_toggle_fvg"),

        _noop("─────────────────────────────────────────"),

        # Liquidity Sweep
        _noop("📌 SWEEP — ложный пробой ликвидности: рынок выбивает"),
        _noop("   стопы выше/ниже уровня и разворачивается обратно."),
        _btn(_check(cfg.smc_use_sweep) + "  Sweep — ложный пробой", p + "smc_toggle_sweep"),

        _noop("─────────────────────────────────────────"),

        # CHOCH
        _noop("📌 CHOCH — смена характера структуры. Первый сигнал"),
        _noop("   разворота ещё до BOS. Ранний вход, но менее точный."),
        _btn(_check(cfg.smc_use_choch) + "  CHOCH — смена структуры", p + "smc_toggle_choch"),

        _noop("─────────────────────────────────────────"),

        # Daily Confluence
        _noop("📌 CONFLUENCE — совпадение с уровнем дневного графика"),
        _noop("   (Pivot Points, Camarilla). Сильно повышает точность."),
