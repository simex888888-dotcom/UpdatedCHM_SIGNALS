...
def _ensure_single_instance():
    ...
    Создаёт lock-файл bot.lock и держит на нём эксклюзивную блокировку.
    Если блокировку уже держит другой процесс — убивает его и запускается сам.
    Работает без ps/pgrep — через /proc, совместимо с bothost.ru и Docker.
    """
    lock_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "bot.lock")
    pid_path  = lock_path.replace(".lock", ".pid")
    _log = logging.getLogger("CHM.Main")

    # Читаем PID предыдущего запуска
    old_pid = None
    try:
    ...
    if old_pid and old_pid != os.getpid():
        # Проверяем жив ли процесс через /proc (работает без ps)
        if os.path.exists(f"/proc/{old_pid}"):
            _log.warning(f"⚠️  Обнаружен второй экземпляр бота (PID {old_pid}) — завершаю его...")
            try:
                ...
                if os.path.exists(f"/proc/{old_pid}"):
                    ...
                    time.sleep(1)
                _log.info(f"✅ Старый процесс PID {old_pid} остановлен.")
            except ProcessLookupError:
                pass
            except PermissionError:
                _log.error(f"❌ Нет прав убить PID {old_pid}. Останови его вручную командой: kill {old_pid}")
    ...
    try:
        ...
        _log.warning(f"Не могу записать PID-файл: {e}")

    # Берём эксклюзивную блокировку файла (страховка)
    try:
        lock_file = open(lock_path, "w")
        fcntl.flock(lock_file, fcntl.LOCK_EX | fcntl.LOCK_NB)
    except BlockingIOError:
        _log.error("❌ Другой экземпляр бота уже держит блокировку. Запуск отменён.")
        raise SystemExit(1)
