...
def kb_gerchik_menu(user: UserSettings) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        _noop("──────────────────────────────────────────────────"),
        _btn("❓ Как работает стратегия — подробнее",    "help_gerchik"),
        _back(),
    ])
...
def gerchik_help_text() -> str:
    return (
        "🎯 <b>СТРАТЕГИЯ ГЕРЧИКА — ПОЛНОЕ РУКОВОДСТВО</b>\n\n"

        "━━ <b>ЧТО ЭТО ТАКОЕ</b> ━━━━━━━━━━━━━━━━━━━━\n"
        "Метод уровневой торговли, разработанный Александром Герчиком.\n"
        "Суть: покупать у уровня поддержки, продавать у уровня сопротивления.\n"
        "Не используются индикаторы — только структура цены и объём.\n\n"

        "━━ <b>ЧТО ТАКОЕ УРОВЕНЬ</b> ━━━━━━━━━━━━━━━━\n"
        "Уровень — это ценовая зона, в которой рынок разворачивался минимум два раза.\n"
        "Бот ищет такие зоны на дневных свечах (таймфрейм 1 день).\n\n"
        "📌 <b>Уровень поддержки</b> — зона снизу, от которой цена отскакивала вверх.\n"
        "   Здесь покупатели перевешивают продавцов. Ищем вход в лонг (покупку).\n\n"
        "📌 <b>Уровень сопротивления</b> — зона сверху, от которой цена отскакивала вниз.\n"
        "   Здесь продавцы перевешивают покупателей. Ищем вход в шорт (продажу).\n\n"
        "📌 <b>Зеркальный уровень</b> — уровень, который раньше был поддержкой, а после\n"
        "   пробоя стал сопротивлением (и наоборот). Такие уровни особенно сильны.\n\n"

        "━━ <b>СИЛА УРОВНЯ</b> ━━━━━━━━━━━━━━━━━━━━━━\n"
        "Бот оценивает каждый уровень по шкале от 1 до 6 звёзд:\n"
        "  +1 звезда — уровень подтверждён касанием\n"
        "  +1 звезда — уровень зеркальный (был поддержкой, стал сопротивлением)\n"
        "  +1 звезда — цена отскочила от уровня с сильным объёмом\n"
        "  +1 звезда — уровень совпадает с дневным или недельным экстремумом\n"
        "  +1 звезда — несколько тестов уровня без пробоя (чем больше, тем лучше)\n"
        "  +1 звезда — уровень подтверждён на старшем таймфрейме (неделя или месяц)\n\n"
        "Бот отправляет сигналы только по уровням с силой <b>2 звезды и выше</b>.\n\n"

        "━━ <b>ПАТТЕРН ВХОДА: БСУ → БПУ-1 → БПУ-2</b> ━━━\n\n"
        "Это трёхшаговая последовательность свечей у уровня:\n\n"
        "🕯 <b>Шаг 1 — Бар Силы Уровня (БСУ)</b>\n"
        "   Свеча, которая <i>создала</i> уровень. Это свеча с явным отталкиванием\n"
        "   от зоны поддержки или сопротивления. Цена подошла к зоне и ушла\n"
        "   от неё с силой — тело свечи закрылось далеко от уровня.\n"
        "   Именно эта свеча «показала» рынку, что здесь есть интерес.\n\n"
        "🕯 <b>Шаг 2 — Первый Бар Подтверждения Уровня (БПУ-1)</b>\n"
        "   Цена вернулась к уровню первый раз после БСУ и снова отскочила.\n"
        "   Это подтверждает, что уровень живой — покупатели/продавцы всё ещё\n"
        "   защищают эту зону. Первое подтверждение даёт уровню доверие.\n\n"
        "🕯 <b>Шаг 3 — Второй Бар Подтверждения Уровня (БПУ-2) — ВХОД</b>\n"
        "   Цена возвращается к уровню второй раз. Уровень снова держит.\n"
        "   Именно здесь бот фиксирует сигнал на вход.\n"
        "   Логика: два отскока — это не случайность. Уровень доказал силу.\n\n"
        "   ✅ Сигнал на покупку: БПУ-2 у уровня поддержки снизу.\n"
        "   ✅ Сигнал на продажу: БПУ-2 у уровня сопротивления сверху.\n\n"

        "━━ <b>ФИЛЬТР ДНЕВНОГО ДИАПАЗОНА</b> ━━━━━━━━\n"
        "Даже при наличии паттерна бот НЕ откроет сделку, если за текущий\n"
        "торговый день цена уже прошла 75% и более от своего среднего\n"
        "дневного диапазона (рассчитывается через ATR — показатель волатильности).\n\n"
        "Почему: если большая часть дневного движения уже позади, входить поздно.\n"
        "Риск неблагоприятного разворота к концу дня слишком высок.\n\n"

        "━━ <b>КУДА СТАВИТСЯ СТОП-ЛОСС</b> ━━━━━━━━━\n"
        "Стоп-лосс размещается <b>за уровнем</b>, от которого произошёл вход.\n"
        "Для лонга: стоп ниже уровня поддержки.\n"
        "Для шорта: стоп выше уровня сопротивления.\n\n"
        "Небольшой буфер добавляется чтобы стоп не срабатывал на рыночном шуме\n"
        "(случайном кратком проколе уровня без реального пробоя).\n\n"

        "━━ <b>ЦЕЛИ ВЫХОДА — TP1 И TP2</b> ━━━━━━━━━\n"
        "Позиция делится на две части:\n\n"
        "🎯 <b>Первая цель (TP1) — 3 расстояния риска (3R)</b>\n"
        "   Закрываем 55% позиции. Это фиксация основной прибыли.\n"
        "   Пример: риск 100 долларов → первая цель даёт 300 долларов.\n\n"
        "🏆 <b>Вторая цель (TP2) — 4 расстояния риска (4R)</b>\n"
        "   Закрываем оставшиеся 45% позиции. Максимальная цель.\n"
        "   Пример: риск 100 долларов → вторая цель даёт 400 долларов.\n\n"
        "Если между точкой входа и следующим уровнем не хватает места для\n"
        "трёхкратного риска — бот не отправит сигнал. Вход нецелесообразен.\n\n"

        "━━ <b>ПЕРЕНОС СТОПА В БЕЗУБЫТОК</b> ━━━━━━━\n"
        "После того как цена достигает первой цели (TP1 = 3R):\n"
        "стоп-лосс переносится на цену входа.\n"
        "Это означает: даже если цена развернётся и пойдёт против нас,\n"
        "сделка закроется в нуле — без потерь.\n"
        "Весь оставшийся ход к TP2 ведётся <b>без риска</b>.\n\n"

        "━━ <b>КАК РАБОТАЕТ СКАНЕР</b> ━━━━━━━━━━━━━\n"
        "• Бот сканирует рынок каждые <b>30 минут</b>.\n"
        "• Анализируется <b>дневной таймфрейм</b> (свечи 1 день).\n"
        "• Проверяются все монеты с суточным оборотом от <b>5 миллионов долларов</b>.\n"
        "• По каждой монете бот ищет уровни, оценивает их силу,\n"
        "  проверяет наличие паттерна и фильтр дневного диапазона.\n"
        "• Повторный сигнал по одной и той же монете не придёт раньше чем через 4 часа.\n"
        "• Если у тебя задан мониторинг конкретной монеты — сканируется только она.\n\n"
        ...
        "💰 Вход:    65 400\n"
        "🛑 Стоп:    64 200  (-1.84%)\n"
        "🎯 TP1:     69 000  (+5.50%  55% позиции)\n"
        "🏆 TP2:     70 200  (+7.34%  45% позиции)\n"
        "📐 R:R = 1:3.0  |  После TP1 — стоп в безубыток\n"
    )
...
def help_text() -> str:
    return (
        ...
        "🎯 <b>Стратегия</b> — три варианта:\n"
        "   📊 <b>Уровни</b> — классические уровни поддержки/сопротивления.\n"
        "   🧠 <b>SMC</b> — институциональный анализ (Order Blocks, ликвидность).\n"
        "   🎯 <b>Герчик</b> — уровневая торговля с паттерном входа БСУ→БПУ-1→БПУ-2.\n"
        "   Подробная справка по стратегии Герчика — кнопка ❓ внутри меню Герчика.\n\n"
